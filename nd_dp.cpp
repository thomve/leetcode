...
// First solution using multidimensional DP: O(mxn)
int minPathSumWithNdDp(vector<vector<int>>& grid) {
...
// Second solution using 1D DP: O(m)
int minPathSumWith1dDp(vector<vector<int>>& grid) {
    int m = grid.size(), n = grid[0].size();
    vector<int> dp(n, 0);
